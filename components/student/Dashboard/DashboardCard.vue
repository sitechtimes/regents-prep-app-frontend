<template>
  <NuxtLink
    ...
    class="my-2 flex h-full w-[calc(85%)] flex-col items-center justify-center overflow-hidden rounded-xl border border-neutral-300 bg-body transition hover:border-neutral-600/50 hover:shadow-lg dark:border-neutral-600 dark:hover:border-neutral-300/50"
    type="button"
  >
    <!-- course information -->
    ...
    <div class="min-h-38 flex h-full w-full flex-col items-center justify-start p-2">
      <h3 class="pb-2 pt-1 text-xl font-bold">Assignments</h3>

      <div v-if="assignments.length > 0" class="flex h-full w-full flex-wrap items-start justify-around gap-3 overflow-hidden px-3 lg:overflow-hidden">
        <NuxtLink
          v-for="assignment in assignments"
          :key="assignment.id"
          :to="`/student/course/${course.id}/${assignment.id}`"
          class="md:h-30 h-34 flex w-full flex-col items-center justify-around overflow-hidden rounded-xl border border-neutral-300 p-3 hover:shadow-lg lg:h-24 dark:border-neutral-600 dark:hover:border-neutral-300/50"
          @click.stop
        >
          ...
          <div class="flex h-full w-full flex-col items-center justify-start gap-3">
            <p class="-mb-3 w-64 overflow-hidden overflow-ellipsis text-nowrap text-center text-lg font-semibold md:text-xl lg:text-xl">{{ assignment.assignment.name }}</p>
