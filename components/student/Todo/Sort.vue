<template>
  <div class="relative flex select-none flex-col items-start justify-start rounded-xl">
    ...
    <button
      ...
      :class="[
        showSorters ? 'border-2 hover:border-neutral-600/50' : 'border-neutral-300 hover:border-neutral-400 dark:border-neutral-700 dark:hover:border-neutral-700',
        currentSorter !== 'due soon' ? 'bg-green-100 dark:bg-green-700' : 'bg-white dark:bg-neutral-800'
      ]"
      type="button"
      @click="showSorters = !showSorters"
    >
      <img class="size-5 dark:invert" src="/ui/sort.svg" aria-hidden="true" draggable="false" />
      <p class="hidden grow translate-y-px lg:block">Sort</p>
    </button>
    ...
    <Transition name="slide-up">
      <div
        v-show="showSorters"
        class="absolute top-10 z-[2] flex w-44 flex-col items-start justify-center gap-1 rounded-xl border border-neutral-300 bg-white p-3 shadow-lg dark:border-neutral-700 dark:bg-neutral-800"
      >
        <button v-for="(sorter, key) in sorters" class="du-label flex w-full cursor-pointer items-center justify-start gap-2" type="button" @click="selectSorter(key)">
          ...
          <span class="translate-y-px text-left capitalize">{{ key }}</span>
        </button>
      </div>
    </Transition>
  </div>
