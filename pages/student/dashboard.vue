  ...
  <div class="flex flex-col items-start justify-start w-full h-full overflow-hidden">
    ...
    <div class="w-full" v-else>
      <Transition name="opacity">
        <div v-if="showNotFound" @click="router.push('/student/dashboard')"
          class="z-50 w-screen h-screen fixed top-0 left-0 bg-[rgba(0,0,0,0.5)] flex items-center justify-center">
          <NotFound type="student" :message="route.query.course ? 'class' : 'assignment'"
            @close="router.push('/student/dashboard')" @click="$event.stopPropagation()" />
        </div>
      </Transition>
      ...
      <div class="h-screen flex flex-col">
        <div class="flex flex-wrap items-start justify-start align-top gap-8" v-if="courses.length > 0">
          <!--prettier-ignore-->
          <StudentDashboardCard @click="router.push(`/student/course/${course.id}`)"
            v-for="course in (courses.filter((course) => !('joinCode' in course)) as StudentCourse[])" :course="course"
            :key="course.id" />
        </div>
        ...
        <div class="flex items-center justify-center h-full w-full overflow-hidden" v-else>
          <div class="flex flex-col items-center gap-4">
            ...
            <button @click="openTheJoin"
              class="mt-4 px-4 py-2 bg-green-accent text-white rounded-lg hover:bg-gray-600 transition duration-200">Add
              Class</button>
          </div>
        </div>
