<template>
  <div class="flex items-center justify-center flex-col w-screen min-h-screen bg-gray py-12">
    <a href="/"><img class="logo h-32 transition duration-500 animate-spin"
    ...
    <div class="flex items-center justify-center flex-col bg-[color:var(--bg-color)] p-4 rounded-3xl mb-4">
      ...
      <form class="login flex items-center justify-center flex-col gap-7 w-full" @submit="loginWithEmail"
        ...
        <div class="relative flex items-start justify-center flex-col gap-1">
          <label class="font-medium" for="email">Your email address <span title="Required"
              class="text-red-500 font-2xl">*</span></label>
          <input
            class="w-96 h-12 rounded-lg border-0 bg-gray px-4 transition duration-500 focus:outline focus:outline-2 focus:outline-[color:var(--primary)] focus:[color:var(--bg-fadded-color)]"
        ...
        <div class="relative flex items-start justify-center flex-col gap-1" v-if="!showLogin">
          <label class="font-medium" for="name">Your name <span title="Required"
              class="text-red-500 font-2xl">*</span></label>
          <input
            class="w-96 h-12 rounded-lg border-0 bg-gray px-4 transition duration-500 focus:outline focus:outline-2 focus:outline-[color:var(--primary)] focus:bg-[color:var(--bg-color)]"
            id="name" type="text" required v-model="name" autocomplete="name" />
          <p class="absolute error font-medium text-red-500" v-show="nameErr.length > 0">{{ nameErr }}</p>
        </div>

        <div class="relative flex items-start justify-center flex-col gap-1">
          <label class="font-medium" for="password">{{ showLogin ? "Your" : "Choose a" }} password <span
              title="Required" class="text-red-500 font-2xl">*</span></label>
          <input
            class="w-96 h-12 rounded-lg border-0 bg-gray px-4 transition duration-500 focus:outline focus:outline-2 focus:outline-[color:var(--primary)] focus:bg-[color:var(--bg-color)]"
        ...
        <div class="relative flex items-start justify-center flex-col gap-1" v-if="!showLogin">
          <label class="font-medium" for="password">Confirm password <span title="Required"
              class="text-red-500 font-2xl">*</span></label>
          <input
            class="w-96 h-12 rounded-lg border-0 bg-gray px-4 transition duration-500 focus:outline focus:outline-2 focus:outline-[color:var(--primary)] focus:bg-[color:var(--bg-color)]"
            id="password" type="password" required v-model="confirmPassword" autocomplete="new-password" />
          <p class="absolute error font-medium text-red-500" v-show="confirmPasswordErr.length > 0">{{
            confirmPasswordErr }}</p>
        </div>

        <button
          class="w-96 h-12 rounded-full border-0 bg-green mt-4 transition duration-500 hover:bg-transparent hover:border-2 hover:border-green"
          type="submit">
          <p class="" v-if="!showLoginAnimation">
            {{ showLogin ? "Log in" : "Sign up" }}
          </p>
          <p class="flex items-center justify-center gap-2" v-else>Loading...
          </p>
        </button>
        <NuxtLink to="/reset-password" class="no-underline font-medium transition duration-500 hover:underline"
          v-if="showLogin">
          Forgot
          password?</NuxtLink>
      </form>
    </div>
    <span class="mb-4" v-show="!showLogin">By signing up, you accept our <a href="/">terms of use</a> and <a
    ...
    <button class="bg-transparent border-0" @click="showLogin ? router.push('?signup=1') : router.push('')">
      <h3 class="m-0 font-medium cursor-pointer transition duration-500 hover:underline">{{ showLogin ? "Sign up now" :
        "Log in" }}</h3>
    </button>
  </div>
</template>
