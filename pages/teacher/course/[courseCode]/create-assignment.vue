  ...
  <div class="mx-auto max-w-lg rounded-lg p-6 shadow-md">
    ...
    <form @submit.prevent="createAssignment" class="space-y-4">
      ...
      <div class="relative">
        ...
        <div @click="guaranteedDropdownOpen = !guaranteedDropdownOpen" class="mt-1 block w-full cursor-pointer rounded-md border-gray-300 px-3 py-2 shadow-sm focus:ring-blue-500 sm:text-sm">
        ...
        <div v-if="guaranteedDropdownOpen" class="absolute z-10 mt-1 max-h-60 w-full overflow-y-auto rounded-md border border-gray-300 bg-white shadow-lg">
          ...
          <div class="flex justify-between bg-gray-50 px-3 py-2">
            <button @click="changeGuaranteedPage('prev')" :disabled="guaranteedCurrentPage === 1" class="text-sm text-blue-500 hover:underline">Previous</button>
            <span class="text-sm">Page {{ guaranteedCurrentPage }}</span>
            <button @click="changeGuaranteedPage('next')" :disabled="guaranteedCurrentPage * batchSize >= questionNumbers.length" class="text-sm text-blue-500 hover:underline">Next</button>
          </div>
        </div>
      </div>
      <div class="relative mt-4">
        <label for="randomQuestions">Random Questions</label>
        <div @click="randomDropdownOpen = !randomDropdownOpen" class="mt-1 block w-full cursor-pointer rounded-md border-gray-300 px-3 py-2 shadow-sm focus:ring-blue-500 sm:text-sm">
        ...
        <div v-if="randomDropdownOpen" class="absolute z-10 mt-1 max-h-60 w-full overflow-y-auto rounded-md border border-gray-300 bg-white shadow-lg">
          ...
          <div class="flex justify-between bg-gray-50 px-3 py-2">
            <button @click="changeRandomPage('prev')" :disabled="randomCurrentPage === 1" class="text-sm text-blue-500 hover:underline">Previous</button>
            <span class="text-sm">Page {{ randomCurrentPage }}</span>
            <button @click="changeRandomPage('next')" :disabled="randomCurrentPage * batchSize >= questionNumbers.length" class="text-sm text-blue-500 hover:underline">Next</button>
          </div>
        </div>
      </div>
      <div class="space-y-2">
      ...
      <button @click="console.log()">Check</button>
