  ...
  <div class="flex h-[90vw] flex-row justify-evenly p-2">
    ...
    <div class="h-full w-[50%] align-middle">
      <div :style="{ width: `${computedDropdownWidth}px` }" class="relative flex h-[40px] items-center justify-center rounded-[20px] border-[1px] border-[#D9D9D9] border-opacity-100">
        <div class="h-full w-full overflow-hidden text-ellipsis whitespace-nowrap pb-2 pl-3 pt-2 pr-3 text-black" @click="toggleDropdown">{{ selectedName || "Student Name" }}</div>
        ...
        <transition name="fade">
          ...
          >
            <div class="flex border-b border-gray-200 p-2 ">
              <div class="flex rounded-[5px] border border-gray-300 bg-white w-[85%] mr-[9px]">
                <input
                  type="text"
                  placeholder="Search Students..."
                  v-model="searchQuery"
                  class="w-[85%] bg-white rounded-[6px] px-2 py-1 text-black focus:outline-none focus:ring focus:ring-gray-200"
                />
                <div class="flex align-middle">
                  <img class="pl-3 pr-2" src="/public/ui/eraser2.svg" alt="" @click="clearSearch" />
                </div>
              </div>
              <div class="flex align-middle rounded-[5px] border border-gray-300 bg-white  h-[35px] w-[35px]">
      ...
      <div class="h-[480px] w-[800px] rounded-[20px] bg-[#D9D9D9] opacity-[100%]"></div>
    </div>
  </div>
  <div>
