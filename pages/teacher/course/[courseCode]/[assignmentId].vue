<template>
  <div class="flex w-full flex-col items-center justify-center gap-8 p-4">
    <div v-if="currentAssignment" class="flex flex-col items-center justify-center">
      <h1 class="text-3xl font-bold">{{ currentAssignment.name }}</h1>
      <p class="mt-1 text-neutral-700">Assigned {{ formatDate(currentAssignment.dateAssigned, currentDate) }}</p>
    ...
    <div class="flex w-2/3 flex-col items-center justify-center gap-8">
      ...
      <div v-if="currentAssignmentStats" class="flex w-full items-center justify-start">
        ...
        <button type="button" @click="currentTab = 'students'" :class="['px-2 text-lg transition-all duration-300 ease-in-out', { underline: currentTab === 'students' }]">Students</button>
